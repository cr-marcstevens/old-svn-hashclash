...
inline void birthday_step(uint32& x, uint32& y, uint32& z)
{
	...
	}
	{
		...
		HASHUTIL5_MD5COMPRESS_STEP(md5_ff, d, a, b, c, z, 0xfd987193, 12);
		HASHUTIL5_MD5COMPRESS_STEP(md5_ff, c, d, a, b, x, 0xa679438e, 17);
		HASHUTIL5_MD5COMPRESS_STEP(md5_ff, b, c, d, a, y, 0x49b40821, 22);
		...
		HASHUTIL5_MD5COMPRESS_STEP(md5_gg, c, d, a, b, y, 0xd8a1e681, 14);
		...
		HASHUTIL5_MD5COMPRESS_STEP(md5_gg, d, a, b, c, x, 0xc33707d6,  9); 
		...
		HASHUTIL5_MD5COMPRESS_STEP(md5_gg, a, b, c, d, z, 0xa9e3e905,  5); 
		...
		HASHUTIL5_MD5COMPRESS_STEP(md5_hh, b, c, d, a, x, 0xfde5380c, 23);
		...
		HASHUTIL5_MD5COMPRESS_STEP(md5_hh, a, b, c, d, z, 0x289b7ec6,  4); 
		...
		HASHUTIL5_MD5COMPRESS_STEP(md5_hh, c, d, a, b, y, 0x1fa27cf8, 16);
		...
		HASHUTIL5_MD5COMPRESS_STEP(md5_ii, c, d, a, b, x, 0xab9423a7, 15);
		...
		HASHUTIL5_MD5COMPRESS_STEP(md5_ii, d, a, b, c, y, 0xfe2ce6e0, 10);
		HASHUTIL5_MD5COMPRESS_STEP(md5_ii, c, d, a, b, block[ 6], 0xa3014314, 15); 
		HASHUTIL5_MD5COMPRESS_STEP(md5_ii, b, c, d, a, z, 0x4e0811a1, 21);
...
struct birthday_thread {
	...
	birthday_thread(bool use_cell_spu = false, int cuda_device_nr = -1)
		: _use_cell_spu(use_cell_spu), _cuda_device_nr(cuda_device_nr), id(id_counter++)
	{}

	void loop_cellcuda(bool single = false)
	{
		...
		while (true)
		{
			uint64 seed;
			{
				...
				if (quit) return;
				if (_use_cell_spu && main_storage.get_collqueuesize() >= 256)
				{
					main_storage.get_birthdaycollisions(collisions, 4);
					// make sure trail1 is always the longest
					for (unsigned i = 0; i < collisions.size(); ++i)
						if (collisions[i].first.len < collisions[i].second.len)
							swap(collisions[i].first, collisions[i].second);
					// sort trails on length
					// so that collision with approx. the same length
					// are processed together in 4-way SIMD
					sort(collisions.begin(), collisions.end(), coll_less());
				}
...
#endif
				cell_fill_trail_buffer(id, seed, work, collisions);
...
#endif // CUDA
						if (_use_cell_spu)
					cout << "Thread " << id << " created (Cell SPU)." << endl;
				else
