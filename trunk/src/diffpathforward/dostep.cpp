...
void dostep0(const path_container_autobalance& container)
{
	...
	hashutil::save_gz(step1diffs, pathsstring("paths0", container.modi, container.modn), binary_archive);
...
void dostep1(const path_container_autobalance& container)
{
	...
	for (unsigned j = 0; j < modn; ++j)
	{
		...
		hashutil::load_gz(step1diffs2, pathsstring("paths0", j, modn), binary_archive);
		cout << "done." << endl;
