...
namespace hashutil {
	...
	std::vector<sdr_carry> hashutil5_sc(0);
	std::vector< std::vector< std::pair<unsigned,unsigned> > > hashutil5_scn(0);
	void hashutil5_init_scn();

	unsigned count_sdrs(uint32 n, unsigned maxw)
	{
		if (hashutil5_scn.size() == 0) hashutil5_init_scn();
		...
		std::vector< std::pair<unsigned,unsigned> >& vl = hashutil5_scn[l];
		std::vector< std::pair<unsigned,unsigned> >& vh1 = hashutil5_scn[h1>>16];
		std::vector< std::pair<unsigned,unsigned> >& vh2 = hashutil5_scn[h2>>16];
		unsigned tot = 0;
	...
	void table_sdrs(std::vector<sdr>& result, uint32 n, unsigned maxw)
	{
		...
		breaknsdr[0]=make_triple(
			&hashutil5_sc[m0].positive,
			&hashutil5_sc[m1>>11].positive,
			&hashutil5_sc[m2>>21].positive);
		breaknsdr[4]=make_triple(
			&hashutil5_sc[m0].positive,
			&hashutil5_sc[m1>>11].positive,
			&hashutil5_sc[m2>>21].negative);
	...
	boost::mutex hashutil5_init_scn_mutex;
	void hashutil5_init_scn()
	{
		boost::lock_guard<boost::mutex> lock(hashutil5_init_scn_mutex);
		if (hashutil5_scn.size()) return;
		std::vector< std::vector< std::pair<unsigned,unsigned> > > hashutil5_scn2(0);
		try {
			load_gz(hashutil5_scn2, "hashutil5_scn", binary_archive);
			hashutil5_scn.swap(hashutil5_scn2);
		} catch(...) {
			hashutil5_scn2.clear();
		}
		if (hashutil5_scn.size()) return;

		sdr temp(0);
		hashutil5_scn2.resize(1<<16);	
		for (temp.mask = 0; temp.mask < 0x10000; ++temp.mask)
			for (temp.sign = 0; temp.sign <= temp.mask; ++temp.sign)
			{
				...
				{
					n += 0x10000;
					if (hashutil5_scn2[n].size() < w+1)
						hashutil5_scn2[n].resize(w+1);
					++hashutil5_scn2[n][w].second;
				} else // positive
				{
					if (hashutil5_scn2[n].size() < w+1)
						hashutil5_scn2[n].resize(w+1);
					++hashutil5_scn2[n][w].first;
				}
			}
		try {
			save_gz(hashutil5_scn2, "hashutil5_scn", binary_archive);
		} catch (...)
		{}
		hashutil5_scn.swap(hashutil5_scn2);
	}

	struct hashutil5_sdr__init {
		hashutil5_sdr__init()
		{
			init_hwtable();
			init_sc();
		...
		void init_sc()
		{
			...
			for (temp.mask = 0; temp.mask < 0x800; ++temp.mask)
				for (temp.sign = 0; temp.sign < 0x800; ++temp.sign)
				{
					...
					{
						n += 0x800;
						if (hashutil5_sc[n].negative.size() < w+1)
							hashutil5_sc[n].negative.resize(w+1);
						hashutil5_sc[n].negative[w].push_back(temp);
					} else // positive
					{
						if (hashutil5_sc[n].positive.size() < w+1)
							hashutil5_sc[n].positive.resize(w+1);
						hashutil5_sc[n].positive[w].push_back(temp);
					}
				}
		}
	};
	hashutil5_sdr__init hashutil5_sdr__init__now;

	void hashutil5_sdr_hpp_init()
	{
		hashutil5_sdr__init here;
	}
