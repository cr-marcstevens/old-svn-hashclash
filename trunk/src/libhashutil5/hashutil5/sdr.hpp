...
#ifndef HASHUTIL5_SIGNED_DIGIT_REPRESENTATION_HPP
#define HASHUTIL5_SIGNED_DIGIT_REPRESENTATION_HPP
...
//	inline unsigned hw(unsigned n) { return hw(uint32(n)); }
	...
	public:
		...
		{
			sdr tmp(*this);
			tmp.mask = hashutil::rotate_left(mask, n);
			tmp.sign = hashutil::rotate_left(sign, n);
			return tmp;
		}
		sdr rotate_right(unsigned n) const
		{
			sdr tmp(*this);
			tmp.mask = hashutil::rotate_right(mask, n);
			tmp.sign = hashutil::rotate_right(sign, n);
			return tmp;
		}
		...
		unsigned hw() const
		{ return hashutil::hw(mask); }
		unsigned hwnaf() const
		{ return hashutil::hwnaf(adddiff()); }
		sdr naf() const
		{ return hashutil::naf(adddiff()); }
...
namespace boost {
	namespace serialization {
		...
		void serialize(Archive& ar, hashutil::sdr& d, const unsigned int file_version)
		{
...
#endif // HASHUTIL5_SIGNED_DIGIT_REPRESENTATION_HPP
